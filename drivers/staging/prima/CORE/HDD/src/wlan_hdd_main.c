...
/*Added by Zhenglq for FTM test. START*/
static int nFwDownload_status = 0;
static int wlan_chip_read_proc(char *page, char **start, off_t off,
                                 int count, int *eof, void *data)
{
      printk("nFwDownload_status =%d\n ",nFwDownload_status);
      return sprintf(page, "nFwDownload_status=%d\n",nFwDownload_status);
}
void wlan_proc_entry_chip_create(void)
{
...
static int hdd_driver_init( void)
{
   ...
   {
      /*Added by Zhenglq for FTM*/
      wlan_proc_entry_chip_create();
