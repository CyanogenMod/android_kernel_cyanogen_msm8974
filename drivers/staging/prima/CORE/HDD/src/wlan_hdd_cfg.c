...
#endif
/*
 * Zhenglq: In case the config file is bad or lost
 * START
 */
VOS_STATUS hdd_copy_config_ini(void)
{
        int ret = 0;
        struct file *fpr = NULL;
        struct file *fpw = NULL;
        mm_segment_t old_fs;
        char buf[512] = {0};
        int len = 0;
        ...
        fpr = filp_open("/system/etc/wifi/WCNSS_qcom_cfg.ini", O_RDONLY, 0);
        if (IS_ERR_OR_NULL(fpr)) {
                hddLog(VOS_TRACE_LEVEL_FATAL, "%s: open source ini file fail\n",__func__);
		fpr = NULL;
                ret = -1;
                goto exit;
        }

        fpw = filp_open("/data/misc/wifi/WCNSS_qcom_cfg.ini", O_WRONLY | O_CREAT, 0666);
        if (IS_ERR_OR_NULL(fpw)) {
                hddLog(VOS_TRACE_LEVEL_FATAL, "%s: create dest ini file fail\n",__func__);
		fpw = NULL;
                ret = -1;
                goto exit;
        }
        while((len = fpr->f_op->read(fpr, buf, sizeof(buf), &fpr->f_pos))> 0) {
                fpw->f_op->write(fpw, buf, len, &fpw->f_pos);
        }
exit:
        if (fpr)
                filp_close(fpr, NULL);
...
VOS_STATUS hdd_parse_config_ini(hdd_context_t* pHddCtx)
{
   ...
   struct file *fpp = NULL;  // Added by Zhenglq

   memset(cfgIniTable, 0, sizeof(cfgIniTable));
   /* in case that data partition bad. Added by Zhenglq */
   fpp = filp_open("/data/misc/wifi/WCNSS_qcom_cfg.ini", O_RDONLY, 0);
   if (IS_ERR_OR_NULL(fpp)) {
      hddLog(VOS_TRACE_LEVEL_FATAL, "zhenglq: file /data/misc/wifi/WCNSS_qcom_cfg.ini does not exist\n");
      fpp = NULL;
      status = hdd_copy_config_ini();
      if(status)
      {
          hddLog(LOGE,"zhenglq1: hdd_copy_config_ini failed\n");
