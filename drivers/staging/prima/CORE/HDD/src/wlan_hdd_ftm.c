...
#define PHY_DOUBLE_CHANNEL_CENTERED 2   // Added by Zhenglq
static unsigned int get_primary_channel(unsigned int center_channel,unsigned int cb)
{
    ...
    switch (cb)
    {
        case PHY_DOUBLE_CHANNEL_LOW_PRIMARY:
            ...
            primary_channel -= 2;
            break;
        case PHY_DOUBLE_CHANNEL_CENTERED:
...
int ftm_channel_to_hz(v_U16_t channel)
{
    if(channel <= 14)
        return 0; /*2.4G*/
    if(channel <= 64 )
                                            ...
                                            v_U16_t txpower)
{
    ...
    if(txpower == 0)
    {
        ...
        nvStatus = vos_nv_getNVBuffer((void **)&nvContents, &nvSize);
        if (!nvStatus  && nvContents)
        {
            channeltype = ftm_channel_to_hz(ftm_status.channel);
            printk(KERN_ERR "GWEI-txpower goal is %u\n",(unsigned int)nvContents->tables.pwrOptimum[channeltype][ftm_status.frameParams.rate].measurement);
            txpower = nvContents->tables.pwrOptimum[channeltype][ftm_status.frameParams.rate].measurement/100;
        }
    }
...
static VOS_STATUS wlan_ftm_priv_set_txrate(hdd_adapter_t *pAdapter,char *txrate)
{
    ...
    /*Added by Zhenglq. TODO: check later*/
    //ftm_status.frameParams.preamble = rate_index_2_preamble_table[rateName_rateIndex_tbl[ii].rate_index].Preamble;

	ftm_status.frameParams.preamble = PHYDBG_PREAMBLE_MIXED ;
	if (rateName_rateIndex_tbl[ii].rate_index <= HAL_PHY_RATE_11AC_DUP_54_MBPS)
	{
		ftm_status.frameParams.preamble 
		= rate_index_2_preamble_table[rateName_rateIndex_tbl[ii].rate_index].Preamble;
    }
