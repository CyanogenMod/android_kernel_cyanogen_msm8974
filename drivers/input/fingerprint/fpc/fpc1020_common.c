...
const fpc1020_setup_t fpc1020_setup_default_1150_a1b1f1 = {
	...
	.capture_finger_up_threshold	= 0,
	.capture_finger_down_threshold	= 2,
	.finger_detect_threshold	= 0x50,
...
int fpc1020_wait_for_irq(fpc1020_data_t *fpc1020, int timeout)
{
	...
	if (result || !timeout) {
		fpc1020->interrupt_done = false;
        if (fpc1020->worker.req_mode == 3) //FPC1020_WORKER_EXIT
	        return -ETIMEDOUT;
        else
		    return 0;
	}

    //dev_warn(&fpc1020->spi->dev, "%s:wait TIMEOUT\n", __func__);
	return -ETIMEDOUT;
}
...
int fpc1020_read_irq(fpc1020_data_t *fpc1020, bool clear_irq)
{
	...
	if (irq_status == FPC_1020_IRQ_REG_BITS_REBOOT) {
		dev_err(&fpc1020->spi->dev,
			...
			, __func__, irq_status);
        while (try_counts > 0 && spi_clk_access_cnt == 0) {
            try_counts--;
            error = fpc1020_reg_access(fpc1020,
                    (clear_irq) ? &reg_clear : &reg_read);
            if (irq_status == FPC_1020_IRQ_REG_BITS_REBOOT) {
		        dev_err(&fpc1020->spi->dev,
			        "%s: unexpected irq_status = 0x%x, try_counts = %d\n"
			        , __func__, irq_status, try_counts);
                error = -EIO;
            } else {
                dev_warn(&fpc1020->spi->dev, "%s: clear success,"
                        "irq_status = 0x%0x, try_counts = %d\n",
                        __func__, irq_status, try_counts);
                error = 0;
                break;
            }
...
sync_err: //RESET fpc1020 device
    ...
    printk(KERN_INFO "%s: prepare switch to TEE, disable spi command\n", __func__);
    return -EACCES;
}
}
...
int fpc1020_wait_finger_present(fpc1020_data_t *fpc1020)
{
	...
	while (spi_clk_access_cnt==0 && !fpc1020->worker.stop_request) {
...
#ifdef FPC1020_TAP_DBG
				dev_dbg(&fpc1020->spi->dev, "is Finger down, input_task\n");
#endif
                fpc1020->down = true;
...
/* -------------------------------------------------------------------- */
int fpc1020_check_finger_present_sum(fpc1020_data_t *fpc1020, int *raw_zones)
{
	int zones = 0;
    ...
    *raw_zones = zones;
    if (fpc1020->nav.read_count == 0)
        fpc1020->nav.read_fst = zones;
    fpc1020->nav.read_count++;
    if (fpc1020->nav.read_count > 5000)
        fpc1020->nav.read_count = 2; /*reset*/
#ifdef FPC1020_TAP_DBG
//	printk("[TAP_DBG] fpc_zones = 0x%x\n", zones&mask);
#endif
	if (zones < 0) {
        dev_warn(&fpc1020->spi->dev, "error zones=%d\n", zones);
		return zones;
    }
	else {
		zones &= mask;
...
int fpc1020_sleep_for_suspend(fpc1020_data_t *fpc1020, bool deep_sleep)
{
	int error = fpc1020_cmd(fpc1020, FPC1020_CMD_ACTIVATE_DEEP_SLEEP_MODE, 0);
	if (error) {
		dev_warn(&fpc1020->spi->dev,
			"%s deep command failed\n", __func__);
		return error;
	}

    dev_warn(&fpc1020->spi->dev,
            "%s suspend deep sleep OK\n", __func__);
    return 0;
}
...
#ifdef FPC1020_DBG
    dev_info(&fpc1020->spi->dev, "Enter sleep mode\n");
#endif
	error = 0;
	sleep_ok = false;
